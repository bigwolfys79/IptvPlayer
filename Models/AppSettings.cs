...
public class AppSettings
{
    ...
    /// далее используется только список. Поле оставлено, чтобы старые файлы
    ...
    /// источника в настройках и далее используется из локального кэша).
    ...
    /// <summary>
    /// Усиление громкости сверх 100% (FFmpeg-фильтр volume): 100 (без
    /// усиления), 125, 150 или 200. Нормализация выравнивает громкость
    /// каналов, но не поднимает её выше исходного уровня — для очень тихих
    /// потоков нужен отдельный gain. Применяется живьём вместе с
    /// нормализацией. По умолчанию — 100 (выключено).
    /// </summary>
    ...
    /// <summary>
    /// Упреждающая буферизация ВИДЕОТЕКИ (сек, 2–15, по умолчанию 8): фильмы
    /// портала стартуют через буфер — 8 сек даёт запас на TCP+TLS при
    /// persistent connections и стабильный старт даже на среднем CDN.
    /// Меньше — быстрее старт, больше — плавнее на нестабильной сети.
    /// </summary>
    ...
    /// включённом родительском контроле: когда суммарное время активного
    /// воспроизведения за день исчерпано, воспроизведение останавливается
    /// и не запускается до полуночи (локального времени).
    /// </summary>
    public int ParentalDailyLimitMinutes { get; set; }

    /// <summary>Дата (локальная, «yyyy-MM-dd»), за которую накоплен счётчик.</summary>
    public string? ParentalWatchedDate { get; set; }

    /// <summary>Просмотрено секунд за день ParentalWatchedDate (счётчик
    /// ведётся в секундах — минутный тик не теряет хвост неполной минуты).</summary>
    ...
    /// <summary>
    /// Пропускать поток через диагностический локальный прокси для
    /// измерения реальной скорости в оверлее статистики (Ctrl+J).
    /// По умолчанию выключено: путь данных при этом буквально меняется
    /// (FFmpeg качает с 127.0.0.1-посредника) — включать на время
    /// диагностики. Применяется при следующем переключении канала.
    /// </summary>
    public bool DiagnosticStreamProxy { get; set; }

    /// <summary>
    /// Временная диагностика (меню «Диагностика», выкл. по умолчанию):
    /// 1) необработанные исключения не роняют процесс — App пишет в лог
    ///    слепок визуального дерева и помечает событие обработанным
    ///    (искали причину зависаний LayoutCycle);
    /// 2) подробный лог EPGService на каждый канал (не только сводка).
    /// </summary>
    ...
    /// VOD-каталог, программа передач для него неприменима, а глобальный
