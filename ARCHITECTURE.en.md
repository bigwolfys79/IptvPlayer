...
- `KinogoSite` — site adapter: category map (`name → /path/`), JS snippets for card parsing (`article.shortStory`, selectors taken from the external reference parser) and pagination (`a[href*='/page/']`), plus the session year filter (POST `xsort=1&xs_field=year&xs_value=N`, same as the parser). The quick-search response parser `ParseLightSearchHtml` handles `a.lightsearch__item` cards (href → film page, "Title (Year)" header, poster from `background-image`). A new site = a new class of the same shape.
...
- `OnlineCinemaCatalogService` — orchestration: the first sync loads 1 page per category; on later opens first pages older than 6 hours refresh in the background; `LoadNextPageAsync` fetches the next page of a category while it does not exceed the pagination total; `SyncYearAsync` applies the session year filter and stores page 1 under the "year N" pseudo-category (the year filter changes the site's first page, so the year is re-requested rather than only filtered locally); `SearchAsync` — the site's quick search (DLE lightsearch): POST `engine/ajax/controller.php?mod=lightsearch` with `q=<query>` (curl → HttpClient, fallback — the hidden WebView2 on the `/search/<query>/` page), found cards are stored in the catalog under the "search" pseudo-category. 2–3 s pauses between pages — anti-ban.
...
- UI: `MainPageViewModel.OnlineCinema.cs` — `SetOnlineCinemaSource` shows the group filter (category = group) and the "Load more" button (`LoadMoreOnlineCinemaCommand`); a selected year syncs with the site in the background (`SyncOnlineCinemaYearAsync`); search: when the debounced local filtering (`FilterChannels`) yields nothing, the query automatically goes to the site (`TriggerOnlineCinemaSiteSearchAsync` → `SearchOnlineCinemaSiteAsync` → `OnlineCinemaCatalogService.SearchAsync`, 2 characters minimum, one request per query per session), results are re-read into the list, and the per-query hit set (`_siteSearchHits`) is consulted by `FilterChannels` — hits stay visible even when the query does not appear in the Russian title. Loading lives in `MainPage.OnlineCinema.cs` (`LoadPlaylistChannelsAsync` early-exits into `LoadOnlineCinemaAsync`). Background collection during playback (`OnlineCinemaBackgroundCollectAsync`, timer in `MainPage.OnlineCinema.cs` with the `AppSettings.OnlineCinemaCollectIntervalSeconds` interval, 120 by default, 30–7200; the interval is re-read on every tick) re-reads the visible list from the DB at most once per `AppSettings.OnlineCinemaListRefreshMinutes` (60 by default, 0 — never); `ReloadOnlineCinemaChannelsAsync` keeps the selected group/genre/year (`RefreshGroups(group, keepFilters: true)`) and moves the selection to the fresh instance of the playing item (by `PageUrl`, with the `IsPlaying` flag) — rebuilding the list neither resets the filters nor kicks back to "All groups".
